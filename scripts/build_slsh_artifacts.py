...
def parse_sources(text: str) -> list[dict]:
    ...
    for i, match in enumerate(starts):
        ...
        if source_id in batch01_kinds:
            ...
            if source_id == "S46":
                audit["NORMALIZED_PUBLISHED_IDENTITY"] = "Vig et al. (2020), Causal Mediation Analysis for Interpreting Neural NLP: The Case of Gender Bias, NeurIPS 2020; arXiv:2004.12265; DOI:10.48550/arXiv.2004.12265."
                audit["PROVENANCE_CORRECTION"] = "PREPRINT_TITLE_VENUE_MIXED_WITH_PUBLISHED_VENUE; RAW_CODEX_IDENTITY_PRESERVED; NORMALIZED_NEURIPS_PUBLISHED_IDENTITY_ADDED."
            row["source_audit"] = audit
...
def main() -> None:
    ...
    access_md = ["# SLSH Source Access and Governance Matrix", "", "This table is generated from the read-only `CODEX_EXTERNAL_RESEARCH_INPUT`; access level is decoupled from source kind and independent verification. Batch 01-Batch 10 audits are explicitly batch-scoped; Batch 10 is limited to S46-S50 and governance disposition is separate with historical provenance preserved.", "", "| ID | Source kind | Access level | Verification actor | Independent verification status | Audit disposition | Support boundary | Direct AI subjectivity evidence | Cross-substrate use | Governance disposition | Evidentiary weight | Admission status | Access evidence |", "|---|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for row in sources:
