...
def main() -> None:
    ...
    for row in sources:
        ...
        access_md.append(f"| {row['id']} | `{row['source_kind']}` | `{row['access_level']}` | `{row['verification_actor']}` | `{row['independent_verification_status']}` | `{audit.get('DISPOSITION', 'NOT_AUDITED_BATCH_01')}` | `{audit.get('SUPPORT_BOUNDARY', 'NOT_AUDITED_BATCH_01')}` | `{audit.get('SOURCE_DOMAIN', 'NOT_AUDITED_BATCH_03')}` | `{audit.get('EVIDENCE_RELATION_TO_AI', 'NOT_AUDITED_BATCH_03')}` | `{audit.get('DIRECT_AI_EVIDENCE', 'NOT_AUDITED_BATCH_02')}` | `{audit.get('DIRECT_AI_SUBJECTIVITY_EVIDENCE', 'NOT_AUDITED_BATCH_01')}` | `{audit.get('CROSS_SUBSTRATE_USE', 'NOT_AUDITED_BATCH_03')}` | `{audit.get('HUMAN_OWNER_REVIEW_NOTE', 'NONE')}` | `{gd.get('disposition', 'NOT_SPECIFIED')}` | `{gd.get('evidentiary_weight', 'NOT_SPECIFIED')}` | `{gd.get('admission_status', 'NOT_SPECIFIED')}` | {row['access_evidence']} |")
