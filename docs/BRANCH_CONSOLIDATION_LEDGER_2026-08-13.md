...
| `MAIN_HEAD_AFTER` | Must equal `MAIN_HEAD_BEFORE`; verified by final read-only ref capture |
| `RESEARCH_HEAD_AFTER` | Final consolidation commit containing this ledger; exact SHA recorded in final handoff |
...
| `tmp-do-not-use` | `cbf9e2e4b0f4bc2be318896684825dc0dcb1000b` | 0 commits / 0 paths | `HISTORICAL_ONLY` | temporary branch with no unique current path; `none` | Retain only as remote history until deletion gate; no content to integrate. |
...
All selected useful branches are integrated into the local formal-research consolidation before any remote deletion. Branches classified `ALREADY_INCLUDED`, `SUPERSEDED` or `HISTORICAL_ONLY` have no remaining useful unique path in the final tree. Remote deletion is permitted only after final QA, manifest/provenance/privacy/secret/stale-evidence scans, independent checkout verification, main-head invariance and final branch-list re-fetch. No deletion is authorized by this ledger alone.
