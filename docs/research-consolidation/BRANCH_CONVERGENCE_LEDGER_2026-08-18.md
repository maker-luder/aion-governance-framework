...
| `test/governance-negative-20260815` | `26de721573a0f4210a1aab8e92dc86dc82be1b9c` | Main comparison showed one unique historical file | Three-line historical negative test selectively materialized | `docs/governance/NEGATIVE_TEST_2026-08-15.md` | Historical governance evidence; not current policy | `HISTORICAL_EVIDENCE` | `SAFE_TO_RETIRE_AFTER_OWNER_EXACT_TARGET_CONFIRMATION` |
| `test/governance-positive-20260815` | `bedcf6a6a56fa6f5c79e3806384f6519817b492a` | Main comparison showed one unique historical file | Three-line historical positive test selectively materialized | `docs/governance/POSITIVE_TEST_2026-08-15.md` | Historical governance evidence; not current policy | `HISTORICAL_EVIDENCE` | `SAFE_TO_RETIRE_AFTER_OWNER_EXACT_TARGET_CONFIRMATION` |
...
| `publication/aion-astra-publication-v0.1-20260817` | `540d190e8b60e92e3b6af94ad8f96d06091c73b3` | Main comparison showed publication/site candidate files | Not absorbed; retained as publication/HOLD candidate | Source branch remains unchanged; `vercel.json` and site files do not enter research convergence | No current red check; publication/deployment remains prohibited | `PUBLICATION_CANDIDATE / HOLD_ARTIFACT` | `NOT_SAFE_TO_RETIRE_UNTIL_DURABLE_HOLD_PRESERVATION_AND_OWNER_CONFIRMATION` |
| `feat/mcp-readonly-interface-20260817` | `d5125c91da3cfa170f0651c0d2d44939fef2f070` | Main comparison showed MCP interface/QA unique content | Not absorbed as approved implementation; provenance uncertain | Quarantine disposition in this ledger; source branch untouched; no runtime or public MCP activation | `PROVENANCE_UNCERTAIN / QUARANTINED_RESEARCH_ARTIFACT` | `UNCERTAIN_PROVENANCE` | `NOT_SAFE_TO_RETIRE_UNTIL_PROVENANCE_RESOLVED_AND_OWNER_CONFIRMATION` |
...
No publication site, `vercel.json`, uncertain MCP interface, private conversation record, newsletter, model/provider/connector, or public deployment surface was materialized.
...
|---|---|---|
| `.github/workflows/research-convergence-consistency.yml` | Both branch targets changed from the obsolete engineering target to `review/four-domain-research-materialization` | `CURRENT_TRUE_DEFECT / 2-LINE SEMANTIC FIX` |
| `scripts/check_research_consolidation_consistency.py` | Current execution allowlist recognizes the authoritative research branch and the single authorized temporary convergence branch; historical branch-head map remains unchanged | `CURRENT_CONTRACT_ALIGNMENT` |
| `tests/research_consolidation/test_research_consolidation_contract.py` | Workflow contract now requires the current research target and rejects the obsolete target | `CURRENT_CONTRACT_ALIGNMENT` |
| `docs/research-consolidation/SOURCE_OF_TRUTH_MAP_V0.1.0.md` | Broken whitepaper pointer replaced with preserved dated lineage plus question-scoped operational projection | `READER_AUTHORITY_REPAIR` |
| `docs/research-consolidation/SOURCE_OF_TRUTH_MAP_V0.1.0.json` | Machine-readable authority wording synchronized; branch-standing authority unchanged | `READER_AUTHORITY_REPAIR` |
...
| `RESEARCH_BRANCH_STATUS.md` | Current observed branch refs separated from final two-branch target and owner-authorized retirement | `CURRENT_VS_TARGET_CORRECTION` |

### Bounded test transition

The first bounded run found two current contract failures because the checker/test still encoded the obsolete engineering branch target. This was classified as `CURRENT_TRUE_DEFECT`, recorded before repair, and fixed minimally. The rerun passed:
