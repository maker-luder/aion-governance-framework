...
> **公開樹修正通知（PR #88）**：原始發布 HEAD `37339d7163cc41a39de64dc19f5d29ab6f1955b6` 曾包含三個 `.zip` 與一個 Git bundle；第一輪修正 HEAD `f13e9d3878b5f4081c65d2e3188d3d98a7c011b6` 移除四個封裝後，遠端 CI 又明確指出 `baseline-root_controls.txt` 含 private-path pattern，因此該 raw traceback 也從公開 candidate 移除。兩次修正都**沒有放寬 scanner**、沒有改寫事故分支或 `main`。詳見 [PUBLICATION_CORRECTION.md](PUBLICATION_CORRECTION.md)。`PUBLICATION_MANIFEST.json` 保留為**原始發布候選的歷史 manifest**，不是目前 PR tree 的即時檔案清單。
...
- [逐命令結果](verification.json)、[基線測試紀錄](baseline-verification.json) 與其餘不含 private-path 的 baseline 文字證據。原始 `baseline-root_controls.txt` 曾存在於初始 evidence publication，但因遠端 public-tree scanner 明確識別 private-path pattern，已從目前 PR candidate 移除；其測試結論仍由 `EXECUTION_HOLD.md` 與 verification records 保留。
...
六份報告中的「未 commit／未 push／remote mutations=0」是上一輪結束時的歷史狀態，不是本 PR 發布後的現況。此次 evidence publication 已建立 candidate commit 與 Draft PR；後續僅做 existing public-tree policy 所要求的封裝／隱私修正。沒有 merge、main write、incident branch rewrite 或 deployment。
...
Codex 交付報告指出本機原件完整保留；本 PR 本身只能證明公開 repository 中的材料，不能獨立驗證未連接的本機副本。公開文字原設計將本機使用者目錄前綴換成 `<LOCAL_HOME>`；遠端 scanner 證明至少一份 raw traceback 仍殘留 private-path，因此該檔已移除，而不是宣稱公開文字消毒已全部由人工先驗證。`PUBLICATION_MANIFEST.json` 保留原始 evidence publication 的逐檔 local/published hash 聲明，但經本次 public-tree 修正後，不再等同目前 Git tree manifest。`DELIVERY_MANIFEST.json` 仍是歷史本機交付 manifest。
