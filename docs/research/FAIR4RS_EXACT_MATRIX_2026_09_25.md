...
| F2 | Software is described with rich metadata. | `PARTIAL` | Root `CITATION.cff`, README, installation, API, interoperability, versioning, provenance, license and current-state documentation provide substantial descriptive metadata. | FAIR4RS states that this metadata must itself be FAIR (F4). Because F4 remains `PARTIAL` and an F1 persistent software identifier is not established, F2 is not treated as fully satisfied. Component-level metadata depth is also uneven. |
