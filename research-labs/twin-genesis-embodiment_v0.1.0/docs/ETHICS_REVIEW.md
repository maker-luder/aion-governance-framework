# Twin Genesis / Embodiment Ethics Review

```text
DOCUMENT = ETHICS_REVIEW
MODULE = twin-genesis-embodiment_v0.1.0
...
## Future opt-in / opt-out requirements

Current v0.1.0 has no user-facing embodiment deployment; therefore no live opt-in mechanism exists.

Before any future interactive embodiment feature may be proposed:

```text
DEFAULT = OFF
EXPLICIT_OPT_IN = REQUIRED
REVOCABLE_OPT_OUT = REQUIRED
PURPOSE_DISCLOSURE = REQUIRED
DATA_SCOPE_DISCLOSURE = REQUIRED
NO_RELATIONSHIP_DERIVED_OPT_IN = REQUIRED
AUDIT_RECORD = REQUIRED
```

Opt-out should disable future processing subject to a separately reviewed retention/deletion policy; it must not silently falsify historical audit records.

## Future review gates

1. ethics definition review;
2. data/privacy review;
3. consent/authorization review;
4. representation/bias review;
5. synthetic-fixture validation;
6. abuse/misinterpretation review;
