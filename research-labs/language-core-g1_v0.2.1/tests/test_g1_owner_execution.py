...
def test_dataset_50_unique_and_private():
    ...
    assert len(rows) == 50
    assert privacy_hits(rows) == []


def test_split_complete():
    rows = load_pairs(DATA)
    parts = deterministic_split(rows)
    assert sum(map(len, parts.values())) == 50
    assert len(parts["holdout"]) > 0


def test_lora_dry_run_reproducible():
    c = LoRAConfig("LIGHT", 8, 16, 0.05)
    assert synthetic_dry_run(c) == synthetic_dry_run(c)


def test_lora_rejects_bad_rank():
    with pytest.raises(ValueError):
        synthetic_dry_run(LoRAConfig("BAD", 7, 16, 0.05))


def test_ablation_strength_limited():
    with pytest.raises(ValueError):
        controlled_low_strength([1.0], [0], 0.2)


def test_random_control_reproducible_and_equal():
    x = synthetic_control_dry_run()
    assert x["status"] == "DRY_RUN_PASS" and x["equal_perturbation_count"]


def test_runtime_candidate_and_kill_switch():
    p = RuntimePolicy()
    assert p.candidate_output("x")["canonical_effect"] == "NONE"
    p.kill()
    with pytest.raises(RuntimeError):
        p.candidate_output("x")
...
def test_localhost_and_traversal():
    p = RuntimePolicy()
    p.validate_endpoint("http://127.0.0.1:11434/api/generate")
    with pytest.raises(PermissionError):
        p.validate_endpoint("https://example.com")
    with pytest.raises(PermissionError):
        safe_child(ROOT, "../escape")


def test_side_effect_schema_has_no_fake_scores():
    report = blank_report("A")
    assert len(DIMENSIONS) == 22 and all(v is None for v in report["dimensions"].values())
