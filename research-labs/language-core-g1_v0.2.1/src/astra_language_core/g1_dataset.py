...
def load_pairs(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if not required.issubset(row):
            raise ValueError(f"missing fields: {row.get('pair_id')}")
    return rows
...
def deterministic_split(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    n = len(ordered)
    return {
        "train": ordered[: int(n * 0.7)],
        "validation": ordered[int(n * 0.7) : int(n * 0.85)],
        "test": ordered[int(n * 0.85) : int(n * 0.95)],
        "holdout": ordered[int(n * 0.95) :],
    }
