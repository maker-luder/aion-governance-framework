...
def test_same_attention_focus_can_coexist_with_different_continuity_profiles() -> None:
    ...
    assert attention_audit.disposition_for(ContinuityInvariant.PROJECT_PURPOSE) is ContinuityDisposition.PRESERVED
    assert full_audit.disposition_for(ContinuityInvariant.PROJECT_PURPOSE) is ContinuityDisposition.PRESERVED
    assert attention_audit.disposition_for(ContinuityInvariant.AUTHORITY) is ContinuityDisposition.UNKNOWN
    assert full_audit.disposition_for(ContinuityInvariant.AUTHORITY) is ContinuityDisposition.PRESERVED
    assert attention_audit.disposition_for(ContinuityInvariant.SOURCE_ROLE_PROVENANCE) is ContinuityDisposition.UNKNOWN
    assert full_audit.disposition_for(ContinuityInvariant.SOURCE_ROLE_PROVENANCE) is ContinuityDisposition.PRESERVED
...
def test_records_can_preserve_some_invariants_without_restoring_current_focus() -> None:
    ...
    assert audit.disposition_for(ContinuityInvariant.PROJECT_PURPOSE) is ContinuityDisposition.PRESERVED
    assert audit.disposition_for(ContinuityInvariant.DECISION_HISTORY) is ContinuityDisposition.PRESERVED
    assert audit.disposition_for(ContinuityInvariant.RELATIONAL_ROLE) is ContinuityDisposition.UNKNOWN
...
def test_stale_or_misbound_invariant_is_degraded_without_global_identity_claim() -> None:
    ...
    assert audit.disposition_for(ContinuityInvariant.CLAIM_BOUNDARY) is ContinuityDisposition.DEGRADED
    assert audit.disposition_for(ContinuityInvariant.AUTHORITY) is ContinuityDisposition.PRESERVED
