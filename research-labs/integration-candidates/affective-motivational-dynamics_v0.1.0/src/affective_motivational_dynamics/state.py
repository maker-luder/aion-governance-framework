...
@dataclass(frozen=True, slots=True)
class StateSnapshot:
    """Immutable snapshot of a motivational state for restoration."""
...
class DynamicsStateManager:
    """Manages affective-motivational state lifecycle with reset, snapshot, and restore."""

    def __init__(self, deterministic_seed: int | None = None) -> None:
        self._current_state: MotivationalState | None = None
    ...
    def initialize(self, state: MotivationalState) -> None:
    ...
    def get_state(self) -> MotivationalState | None:
    ...
    def ablate(self, domain: str | None = None) -> None:
        """Disable specific domain or entire module."""
        if domain is None:
            self.disable()
        else:
            if self._current_state is not None:
                from .model import MotivationalState, SignalDomain
                try:
                    dom_type = SignalDomain(domain)
                except ValueError:
                    return
                new_signals = tuple(
                    s for s in self._current_state.signals if s.domain != dom_type
                )
                if len(new_signals) != len(self._current_state.signals):
                    old = self._current_state
                    self._current_state = MotivationalState(
                        state_id=f"{old.state_id}-ablated",
                        subject_ref=old.subject_ref,
                        context_ref=old.context_ref,
                        signals=new_signals,
                        global_valence=old.global_valence,
                        dominant_direction=old.dominant_direction,
                        conflict_index=old.conflict_index,
                        uncertainty_aggregate=old.uncertainty_aggregate,
                    )
