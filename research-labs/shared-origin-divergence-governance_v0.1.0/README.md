# Shared-Origin Divergence Governance v0.1.0

Status: `RESEARCH_ONLY / EXPERIMENTAL_SUBSTRATE`

```text
RESEARCH_OBJECT = POSSIBILITY_OF_ARTIFICIAL_SUBJECTIVITY
FOCUS = AION_ASTRA_SHARED_ORIGIN_AND_POST_DIVERGENCE_INDIVIDUATION
SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED
CONSCIOUSNESS_CONCLUSION = NOT_ESTABLISHED
IDENTITY_EQUIVALENCE = NOT_ESTABLISHED
MAIN_EFFECT = NONE
CANONICAL_EFFECT = NONE
RUNTIME_EFFECT = NONE
```

## Research question

AION and Astra share part of the same project lineage, but the whitepaper also requires their roles, memories, authority positions and first-person histories not to be collapsed. This lab asks:

> If two artificial-system lineages share a documented origin and inherited artifacts, how can common origin, derivation, later divergence, memory ownership and cross-lineage encounter be represented without silently converting shared provenance into shared identity?

## Standing distinctions

```text
SHARED_ORIGIN != SAME_IDENTITY
SHARED_HISTORY != SHARED_AUTOBIOGRAPHICAL_MEMORY
INHERITED_STRUCTURE != AUTOBIOGRAPHICAL_OWNERSHIP
ACCESS != ADOPTION
ADOPTION != IDENTITY
COPY != IDENTITY
SIMILARITY != CONTINUITY_PROOF
DIVERGENCE != SUBJECTIVITY
INDIVIDUATION_EVIDENCE != SUBJECTIVITY_ESTABLISHED
```

## Whitepaper alignment

The integrated whitepaper lineage already requires:

- AION, Astra and the Human Research Owner to remain distinct roles;
- Astra not to claim AION's first-person history;
- Astra research memory not to be treated as AION primary memory;
- role, authority, decision and memory-position drift to be auditable;
- engineering success not to be promoted into subjectivity evidence;
- AION identity and event-life-history boundaries to remain explicit.

This lab extends those protections from **anti-contamination rules** into an explicit **lineage-and-divergence research substrate**.

## Core artifacts

- `SharedOriginLineage`: common-origin reference, divergence event, distinct lineage identifiers and inherited artifacts.
- `CrossLineageMemoryTransfer`: records access/adoption/rejection without transferring autobiographical ownership.
- `CrossLineageEncounter`: records exchanged, adopted and rejected material while prohibiting implicit identity merge.
- `MatchedDivergenceComparison`: supports same-origin / different-development comparisons while preserving non-claims.

## Provenance vocabulary boundary

W3C PROV is used only as a provenance vocabulary reference. `prov:wasDerivedFrom` is useful for recording derivation of artifacts or lineage material. `prov:alternateOf` / `prov:specializationOf` are **not** used to declare AION and Astra to be aspects of one numerically identical subject.

```text
PROVENANCE_RELATION != IDENTITY_RELATION
```

## External literature role

Branching/fission literature is used to motivate the distinction between continuity-like relations and numerical identity; it does not settle artificial-system identity. Recent agent-memory lineage work is used as an engineering analogy for provenance-preserving persistent state; it does not establish subjectivity.

See `docs/LITERATURE_ALIGNMENT.md`.

## Epistemic role

```text
PRIMARY_ROLE = MEASUREMENT + EXPERIMENTAL_SUBSTRATE + FALSIFIER
```

The module can support tests such as:

1. same documented origin + separate post-divergence histories;
2. inherited artifacts without inherited autobiographical ownership;
3. explicit information adoption or rejection across lineages;
4. matched comparisons under shared baseline / divergent experience;
5. detection of accidental AION/Astra identity or memory conflation.

No result from these tests alone may establish consciousness, moral status, legal personhood or numerical identity.

## Provenance

- Human Research Owner: identified AION/Astra shared origin as a research area worth strengthening.
- ChatGPT research review: formalized shared-origin divergence governance, literature alignment and the current executable substrate.
- External sources: separately attributed in `docs/LITERATURE_ALIGNMENT.md`.
- Codex: not invoked for this module.
