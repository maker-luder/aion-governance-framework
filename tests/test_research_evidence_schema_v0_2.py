...
def test_v2_candidate_accepts_exact_main_native_protocol_fields() -> None:
    assert list(_validator(V2).iter_errors(_v2_record())) == []


def test_v2_candidate_rejects_research_ab6_dimension_leakage() -> None:
    record = _v2_record()
...
def test_v2_schema_contains_no_ab6_dimension_vocabulary() -> None:
    schema_text = V2.read_text(encoding="utf-8")
    assert "AB.6" not in schema_text
...
def test_duplicate_provenance_refs_fail_closed() -> None:
