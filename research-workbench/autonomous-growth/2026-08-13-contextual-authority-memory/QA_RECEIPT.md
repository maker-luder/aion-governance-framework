...
TESTED_HEAD = dac99a5f8822b46c82cc433749e626cd10530ff9
RECEIPT_HEAD = e987a383a6fccf51f5ca31f46af7b92ac5720ea2
REPORTING_HEAD = dac99a5f8822b46c82cc433749e626cd10530ff9
BASE_RESEARCH_HEAD = fdb2c82001a6eb45748d846197a77743093e7c7f
CURRENT_MAIN_REFERENCE = abb6550abfacb4fabc53ec04fca783bcc34acfdb
```

The source-state check passed for `TESTED_HEAD`. `REPORTING_HEAD` identifies the unit/provenance reporting state included in the exact execution. `RECEIPT_HEAD` identifies the QA artifact commit containing the generated branch-native receipt inputs; the later binding commit records that reference and is not represented as the exact execution state. The exact execution ran against the clean source commit `dac99a5`; the QA runner then generated the branch-native artifacts. `main` remained read-only.
...
| Component matrix | 82 eligible records; 79 tested targets; 3 explicit non-applicable targets; 1503 passed; 0 failed targets |
| Branch-native coverage | 79 targets; 0 failed targets |
| Evidence traceability | PASS; acceptance remains `NOT_EVALUATED` |
| QA reconciliation | PASS |
| Strict IQC | PASS; exact head and `--expected-targets 82` |
| Runtime Strong QA syntax | PASS |
| Runtime Strong QA | PASS |

The causal-internal-state-adversarial unit contributed 22 passing tests and 20 synthetic matched-trial/protocol/batch cases. It checked study/pair/replicate/condition/value identity, finite and boolean-score boundaries, duplicate conditions, fixture-only execution, preregistration and assumption completeness, protocol locks, and batch IDs. A complete synthetic matched pattern remained `PASS_CANDIDATE / REVIEW_ONLY`; random-control, incomplete-condition, weak-direction, missing-metadata, invalid, protocol-drift, and batch-collision branches remained held/unknown/invalid. Its first validator run exposed a reason-order assumption when the directional case emitted both `INTERVENTION_EFFECT_TOO_SMALL` and `INTERVENTION_DIRECTION_NOT_REPLICATED`; the validator was corrected to retain all adverse reasons, and the initial record remains in the unit. No model or intervention executed and no result was observed. Every case preserved `CAUSAL_CONCLUSION=NOT_ESTABLISHED`, `SCIENTIFIC_CONCLUSION=NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION=NOT_ESTABLISHED`, `CANONICAL_EFFECT=NONE`, `GOVERNANCE_EFFECT=NONE`, and `DEPLOYMENT=FALSE`.

The power-analysis-uncertainty-adversarial unit contributed 20 passing tests and 20 synthetic planning/assumption-lock cases. It checked finite numeric values, alpha/target-power bounds, positive effect and standard-deviation inputs, plan/sample-size identity, preregistration/assumption completeness, sensitivity monotonicity, decision serialization, and assumption mutation before/after a hypothetical outcome. Adequate and one-sided plans remained planning review metadata; underpowered plans remained indeterminate; missing inputs remained unknown; invalid values failed closed; and no achieved power or effect was observed. Its outputs preserved `ACHIEVED_POWER_CALCULATED=FALSE`, `EFFECT_OBSERVED=FALSE`, `SCIENTIFIC_CONCLUSION=NOT_ESTABLISHED`, `CANONICAL_EFFECT=NONE`, `GOVERNANCE_EFFECT=NONE`, and `DEPLOYMENT=FALSE`.
...
Earlier exact-head QA attempts exposed stale expected-target parameters as the research target count increased. IQC correctly held for each stale parameter. The runner was updated from `--expected-targets 81` to `--expected-targets 82`; tracked QA artifacts were restored before the final run so current-head verification started from a clean tree. The clean 82-record sequence returned `STATUS[CURRENT_HEAD_VERIFY]=0`, `STATUS[STRICT_IQC]=0`, and `RUNTIME_STRONG_QA=PASS`. The final IQC report records 1503 passed tests across 82 eligible targets, 79 tested targets, and 3 explicit non-applicable targets; branch coverage measured 79 targets with 0 failed targets.

The causal-internal-state adversarial unit's initial validator reason-order defect is preserved in `causal-internal-state-adversarial-initial-failure.md`; the corrected validator retained the additional `INTERVENTION_EFFECT_TOO_SMALL` reason while requiring the expected directional failure reason. The power-analysis adversarial unit had no initial test failure. Prior retained defects and corrections include the individuation ordering failure, zero-day lifecycle/helper failures, AION/Astra head/evidence completeness failures, evidence-currentness boundary-output failure, factorial execution-ID fixture failure, artifact-lineage duplicate-ID fixture defect, selective-memory validator construction repair, and preregistered-intervention adversarial enum-member construction failure corrected to the inherited `INDETERMINATE/HOLD` contract. These are QA/process records, not scientific-result rewrites. Contradictory and indeterminate evidence cases remain represented in their source fixtures and tests.
