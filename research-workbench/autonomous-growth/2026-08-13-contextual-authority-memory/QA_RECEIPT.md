...
TESTED_HEAD = 114c7def7bef25f5e08af387e4b475051a1caae2
RECEIPT_HEAD = 2af26ce6a2f21b13a9d099d88051980aa7097a24
REPORTING_HEAD = 114c7def7bef25f5e08af387e4b475051a1caae2
BASE_RESEARCH_HEAD = 7dddd041cfebeaa3cb4a29b60f858a007d231a35
CURRENT_MAIN_REFERENCE = abb6550abfacb4fabc53ec04fca783bcc34acfdb
```

The source-state check passed for `TESTED_HEAD`. `RECEIPT_HEAD` identifies the later QA artifact/receipt commit and `REPORTING_HEAD` identifies the preceding provenance/reporting commit; neither is presented as the exact execution state. The exact execution ran against the clean source commit `114c7de`; the QA runner then generated the branch-native QA artifacts.
...
| Component matrix | 79 eligible records; 76 tested targets; 3 explicit non-applicable targets; 1437 passed; 0 failed targets |
| Branch-native coverage | 76 targets; 0 failed targets |
| Evidence traceability | PASS; acceptance remains `NOT_EVALUATED` |
| QA reconciliation | PASS |
| Strict IQC | PASS; exact head and `--expected-targets 79` |
...
Earlier exact-head QA attempts exposed stale expected-target parameters as the research target count increased. IQC correctly held for each stale parameter. The runner was updated from `--expected-targets 78` to `--expected-targets 79`; tracked QA artifacts were restored before the final run so current-head verification started from a clean tree. The clean 79-record sequence returned `STATUS[CURRENT_HEAD_VERIFY]=0`, `STATUS[STRICT_IQC]=0`, and `RUNTIME_STRONG_QA=PASS`. The final IQC report records 1437 passed tests across 79 eligible targets and coverage over 76 tested targets.
