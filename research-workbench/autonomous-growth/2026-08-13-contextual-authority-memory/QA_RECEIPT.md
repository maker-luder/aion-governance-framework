...
TARGET_HEAD = 43aeee20ba9c642af55d30405f5b39f1e86129c9
...
| Component matrix | 64 eligible records; 61 tested targets; 3 explicit non-applicable targets; 1105 passed; 0 failed targets |
| Branch-native coverage | 61 targets; 0 failed targets |
| Evidence traceability | PASS; acceptance remains `NOT_EVALUATED` |
| QA reconciliation | PASS |
| Strict IQC | PASS; exact head and `--expected-targets 64` |
...
Earlier exact-head QA attempts exposed stale expected-target parameters as the research target count increased. IQC correctly held for each stale parameter. The runner was updated from `--expected-targets 63` to `--expected-targets 64`; tracked QA artifacts were restored before the final run so current-head verification started from a clean tree. The final sequence returned `STATUS[CURRENT_HEAD_VERIFY]=0`, `STATUS[STRICT_IQC]=0`, and `RUNTIME_STRONG_QA=PASS`. The final IQC report records 1105 passed tests across 64 eligible targets and coverage over 61 tested targets.

These are QA/process repairs, not scientific-result rewrites. No source or component test failure occurred in the final sequence. Contradictory and indeterminate evidence cases remain represented in the evidence-admission and individuation-threshold fixtures and tests. The individuation unit's initial ordering failure remains preserved in its initial-failure record.
