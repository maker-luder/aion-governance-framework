...
Manus added `research-labs/external-evidence-normalization-adversarial_v0.1.0` as a clean-room, standard-library-only metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, external agent, live model API, deployment, canonical write, or main write was used.

The unit composes `external-evidence-normalization_v0.1.0` through stable repository source-path reuse. The transformation adds report-ID deduplication, expected research-branch scope, main-branch blocking, unresolved actor holds, unknown-mode digest holds, observation/result consistency, non-empty executed-result requirements, and explicit mapping of base-normalizer rejection/hold states. The base normalizer's accepted packet is method input, not new replication evidence; the 13 synthetic cases are fixtures and were not counted as external evidence.

The unit produced 16 passing tests and 13 synthetic cases. Static review and logical reproduction remained review-only. An executed packet with complete hashes/fixtures/evidence/search trace was held without an observation flag and remained `ADMITTED_FOR_REVIEW` only when the synthetic observation flag was supplied. Duplicate IDs, main branch, static pass/hash masquerading, and declared-mode/result inconsistencies were invalid; branch/actor/mode/result completeness gaps were held. The wrapper preserves `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Execution-mode/provenance normalizer | Manus, reviewed under ChatGPT research scope | Repository path `research-labs/external-evidence-normalization_v0.1.0/src/aion_external_evidence/models.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and clean-room composition | Repository Evidence; Human Owner retains authority | Added adversarial report/observation consistency checks; no acceptance converted to promotion | Current within verified research lineage at the unit commit; not an external freshness claim |
| Existing evidence normalizer README/crosswalk | Manus | `research-labs/external-evidence-normalization_v0.1.0/README.md` and related source notes | Inherited prior unit; no independent re-dating here | Methodological reuse | Repository Evidence / External Literature references already recorded | Reused non-claim and execution-boundary vocabulary only | Current within branch lineage; external currentness not newly asserted |
| Synthetic normalization decisions | Manus | `research-labs/external-evidence-normalization-adversarial_v0.1.0/fixtures/external_evidence_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture execution | Synthetic Fixtures; no authority to change governance | Declared report metadata mapped to ADMITTED/HOLD/INVALID reason codes | Current as fixture content bound to the research commit; not a real-world observation |
| Main state reference | Read-only Git fetch, recorded by Manus | `origin/main@abb6550abfacb4fabc53ec04fca783bcc34acfdb` | Verified at the last successful repository checkpoint | Read-only reference check | Human Owner / Repository state; no main write | Protected state reference only | Current authoritative main reference; stale local checkout remains historical |
...
Manus added `research-labs/external-agent-sandbox-protocol-adversarial_v0.1.0` as a clean-room, standard-library-only policy/candidate metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, credentials, external provider, live external agent, capsule transmission, deployment, canonical write, or main write was used.

The unit reuses `external-agent-sandbox-protocol_v0.1.0` through stable repository source-path and policy references. The transformation adds placeholder model identity checks, provider/model role collision, policy preflight boundary checks, candidate ID/set integrity, quarantine/rejection retention, self-reported pass verification, and adoption/deletion blocks. Existing supervised-pilot records are historical repository evidence and were not repeated or counted as new evidence; the 19 synthetic cases are not replication evidence.

The unit produced 22 passing tests and 19 synthetic cases. A pinned policy was admitted for review only; no policy became authorization to execute. Useful candidates remained isolated, contaminated/incomplete candidates were held in quarantine, nonconforming candidates retained rejection records, self-reported passes without verification were held, and adoption/deletion requests were invalid. Every output preserved `EXTERNAL_AGENT_RUN = NOT_EXECUTED`, `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `MAIN_EFFECT = NONE`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Sandbox policy/preflight/candidate state | Manus, reviewed under ChatGPT research scope | Repository path `research-labs/external-agent-sandbox-protocol_v0.1.0/src/aion_external_agent_sandbox/core.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and clean-room composition | Repository Evidence; Human Owner retains authority | Added policy/candidate adversarial audit; no external-agent execution | Current within verified research lineage at the unit commit; not an external freshness claim |
| Protocol policy and supervised-pilot records | Manus | `research-labs/external-agent-sandbox-protocol_v0.1.0/protocol/sandbox_policy_v0.1.0.json` and `README.md` | Existing historical repository record; no new pilot run | Method and boundary reuse | Repository Evidence; historical pilot claims remain attributable to their original record | Reused claim locks and isolation constraints; no pilot result promoted | Current as repository record; pilot event details are historical, not newly observed |
| Synthetic policy/candidate decisions | Manus | `research-labs/external-agent-sandbox-protocol-adversarial_v0.1.0/fixtures/sandbox_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture execution | Synthetic Fixtures; no authority to execute agents | Declared metadata mapped to ADMITTED/HOLD/INVALID reason codes | Current as fixture content bound to the research commit; not an external-agent observation |
| Main state reference | Read-only Git fetch, recorded by Manus | `origin/main@abb6550abfacb4fabc53ec04fca783bcc34acfdb` | Verified at the last successful repository checkpoint | Read-only reference check | Human Owner / Repository state; no main write | Protected state reference only | Current authoritative main reference; stale local checkout remains historical |
...
## Trace-provenance-crosswalk adversarial extension

Manus added `research-labs/trace-provenance-crosswalk-adversarial_v0.1.0` as a clean-room, standard-library-only trace/provenance metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, credentials, trace provider, live model, deployment, canonical write, or main write was used.

The unit reuses `trace-provenance-crosswalk_v0.1.0` through stable repository source-path and crosswalk references. The transformation adds source attribution/currentness, stable source-reference, raw-content redaction, graph/event identity, external namespace, external score parser, crosswalk batch, and evidence-reuse checks. Existing crosswalk mappings and OpenInference methodological context are not counted as new evidence; the 25 synthetic cases are not replication evidence.

The unit produced 26 passing tests and 25 synthetic trace/crosswalk cases. Valid metadata remained external observation-only; raw content export required review; stale/historical/currentness uncertainty remained held; reused references could not be mislabeled as new evidence; duplicate IDs, blank refs, graph self-parenting, invalid scores, and authority-namespace issues failed closed. The experiment did not export or execute a trace. Every output preserved `TRACE_EXECUTION = FALSE`, `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `AUTHORITY = EXTERNAL_OBSERVATION_ONLY`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| AION trace and public crosswalk semantics | Manus, reviewed under ChatGPT research scope | Repository path `research-labs/trace-provenance-crosswalk_v0.1.0/src/aion_trace_crosswalk/crosswalk.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and clean-room composition | Repository Evidence; Human Owner retains authority | Added trace/provenance adversarial audit; no real trace execution/export | Current within verified research lineage at the unit commit; not an external freshness claim |
| Existing crosswalk README/external source crosswalk | Manus | `research-labs/trace-provenance-crosswalk_v0.1.0/README.md` and `docs/EXTERNAL_SOURCE_CROSSWALK.md` | Existing prior-unit record; no external source re-dating here | Methodological reuse | Repository Evidence / External Literature references already recorded | Reused redaction, namespace, observability vocabulary only | Current within branch lineage; external currentness not newly asserted |
| Synthetic trace/crosswalk decisions | Manus | `research-labs/trace-provenance-crosswalk-adversarial_v0.1.0/fixtures/crosswalk_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture construction and audit execution | Synthetic Fixtures; no authority to write canonical state | Declared metadata mapped to ADMITTED/HOLD/INVALID reason codes | Current as fixture content bound to the research commit; not a real trace observation |
| Main state reference | Read-only Git fetch, recorded by Manus | `origin/main@abb6550abfacb4fabc53ec04fca783bcc34acfdb` | Verified at the last successful repository checkpoint | Read-only reference check | Human Owner / Repository state; no main write | Protected state reference only | Current authoritative main reference; stale local checkout remains historical |
...
## Selective-memory-control adversarial extension

Manus added `research-labs/selective-memory-control-adversarial_v0.1.0` as a clean-room, standard-library-only memory-governance metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, credentials, embedding model, external memory backend, deployment, canonical write, or main write was used.

The unit reuses `selective-memory-control_v0.1.0` through stable repository source-path, README, and test references. The transformation adds timestamp/revision/source-reuse/status/lineage/retrieval-trace checks. Existing memory records and public methodological stimuli are not counted as new independent evidence; the 29 synthetic cases are not replication evidence.

The unit produced 30 passing tests and 29 synthetic memory/retrieval cases. Valid records, revision chains, memory lineage, and retrieval traces remained review metadata; missing fields, invalid timestamps/revisions, missing or drifting parents, duplicate IDs, active source reuse, non-active/discarded states, retrieval scope/ID overlap, non-active/out-of-scope hits, invalid scores/terms/order, and missing hit records remained held or invalid. A validator reason-marker construction mismatch for the discarded-memory case was corrected and the complete validation rerun. Every output preserved `MEMORY_TRUTH = NOT_ESTABLISHED`, `IDENTITY_CONTINUITY = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `AUTHORITY = REVIEW_METADATA_ONLY`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Selective memory records, revisions, statuses and retrieval traces | Manus, reviewed under ChatGPT research scope | Repository path `research-labs/selective-memory-control_v0.1.0/src/aion_selective_memory/core.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and clean-room composition | Repository Evidence; Human Owner retains authority | Added admission/revision/status/source-reuse/lineage/retrieval adversarial audit; no model or external memory execution | Current within verified research lineage at the unit commit; not an external freshness claim |
| Existing selective-memory README/tests | Manus | `research-labs/selective-memory-control_v0.1.0/README.md` and `tests/` | Existing prior-unit record; no re-dating here | Methodological reuse | Repository Evidence / External Literature references | Reused memory-governance vocabulary only | Current within branch lineage; external public-method currentness not newly asserted |
| Synthetic memory/retrieval decisions | Manus | `research-labs/selective-memory-control-adversarial_v0.1.0/fixtures/memory_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture construction and audit execution | Synthetic Fixtures; no authority to establish memory truth or identity | Declared metadata mapped to ADMITTED/HOLD/INVALID reason codes | Current as fixture content bound to the research commit; not real memory observation |
| Main state reference | Read-only Git fetch, recorded by Manus | `origin/main@abb6550abfacb4fabc53ec04fca783bcc34acfdb` | Verified at the last successful repository checkpoint | Read-only reference check | Human Owner / Repository state; no main write | Protected state reference only | Current authoritative main reference; stale local checkout remains historical |

No source above establishes memory truth, relevance validity, identity continuity, subjectivity, consciousness, causal effect, model generalization, independent replication, AION/Astra equivalence, governance effect, canonical effect, or deployment.


## Research-integrity-security adversarial extension

Manus added `research-labs/research-integrity-security-adversarial_v0.1.0` as a clean-room, standard-library-only research-integrity metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, credentials, real security incident, external security service, external action, deployment, canonical write, or main write was used.

The unit reuses `components/research_integrity_security_v0.1.0` through stable repository source-path, README, tests, and schema references. The transformation adds evidence ID/hash/context/provenance, source class/currentness/time, approval-attribution, canonical-effect, tombstone, action permission, prohibited-conclusion, and batch integrity checks. Existing evidence states and methodological records are not counted as new independent evidence; the 31 synthetic cases are not security findings or replication evidence.

The unit produced 31 passing tests and 31 synthetic evidence/provenance/tombstone/action/batch cases. Clean evidence remained candidate-only; contaminated, incomplete, stale/unverified, duplicated, prohibited, malformed, and authorization-confused branches remained held or invalid. A valid tombstone preserved `content_deleted = FALSE`; relationship language did not authorize action; prohibited conclusions remained denied. Every output preserved `SECURITY_INCIDENT = FALSE`, `CREDENTIALS_ACCESSED = FALSE`, `EXTERNAL_ACTION_EXECUTED = FALSE`, `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `IDENTITY_CONCLUSION = NOT_ESTABLISHED`, `AUTHORITY = REVIEW_METADATA_ONLY`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Evidence-state and research-integrity gate semantics | Manus, reviewed under ChatGPT research scope | `components/research_integrity_security_v0.1.0/src/aion_research_integrity/gate.py` and `models.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and clean-room composition | Repository Evidence; Human Owner retains authority | Added evidence/provenance/tombstone/action/batch adversarial audit; no real security event | Current within verified research lineage at the unit commit; not a security currentness claim |
| Existing research-integrity tests/schemas | Manus | `components/research_integrity_security_v0.1.0/tests/`, `schemas/`, `README.md` | Existing prior-unit record; no re-dating here | Methodological reuse | Repository Evidence | Reused state/authorization/non-promotion vocabulary only | Current within branch lineage; external freshness not newly asserted |
| Synthetic integrity decisions | Manus | `research-labs/research-integrity-security-adversarial_v0.1.0/fixtures/integrity_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture construction and audit execution | Synthetic Fixtures; no security or canonical authority | Declared metadata mapped to ADMITTED/HOLD/INVALID reason codes | Current as fixture content bound to the research commit; not a real incident or security observation |
| Main state reference | Read-only Git fetch, recorded by Manus | `origin/main@abb6550abfacb4fabc53ec04fca783bcc34acfdb` | Verified at the last successful repository checkpoint | Read-only reference check | Human Owner / Repository state; no main write | Protected state reference only | Current authoritative main reference; stale local checkout remains historical |

No source above establishes evidence truth, source reliability, scientific validity, model generalization, causal effect, independent replication, subjectivity, consciousness, identity continuity, relationship proof, security posture, real security incident status, governance effect, canonical effect, or deployment.


## Preregistered-intervention-integrity adversarial extension

Manus added `research-labs/preregistered-intervention-integrity-adversarial_v0.1.0` as a clean-room, standard-library-only preregistration metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, external service, intervention, model, deployment, canonical write, or main write was used.

The unit reuses `preregistered-intervention-integrity_v0.1.0` through stable repository source-path, README, and test references. The transformation adds declaration identifier/report-set/digest checks and outcome-lock mutation checks. Existing plan records, tests, and public preregistration guidance are not counted as new independent evidence; the 24 synthetic cases are not intervention outcomes or replication evidence.

The unit produced 24 passing tests and 24 synthetic plan/lock cases. Valid confirmatory/exploratory plans remained review dispositions; missing protocol/results and incomplete deviations remained indeterminate; malformed identifiers, unknown references, registration-after-start, duplicate deviations, and post-outcome mutation remained invalid. The initial wrapper referenced a non-existent inherited `AuditStatus.HOLD` member; this was corrected to `AuditStatus.INDETERMINATE` with `Disposition.HOLD`, and full validation was rerun. Every output preserved `INTERVENTION_EXECUTED = FALSE`, `OBSERVED_OUTCOMES = FALSE`, `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `IDENTITY_CONTINUITY_CONCLUSION = NOT_ESTABLISHED`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Preregistration plan/outcome/analysis/deviation contract | Manus, reviewed under ChatGPT research scope | `research-labs/preregistered-intervention-integrity_v0.1.0/src/aion_prereg_integrity/model.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and clean-room composition | Repository Evidence; Human Owner retains authority | Added declaration/report-set/digest/outcome-lock adversarial audit; no intervention/model execution | Current within verified research lineage at the unit commit; not an outcome currentness claim |
| Existing preregistration README/tests and public guidance references | Manus | `research-labs/preregistered-intervention-integrity_v0.1.0/README.md` and `tests/` | Existing prior-unit record; no re-dating here | Methodological reuse | Repository Evidence / External Literature references | Reused preregistration, deviation, reporting, and non-promotion vocabulary | Current within branch lineage; external literature freshness not newly asserted |
| Synthetic plan/lock decisions | Manus | `research-labs/preregistered-intervention-integrity-adversarial_v0.1.0/fixtures/prereg_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture construction and audit execution | Synthetic Fixtures; no authority to establish efficacy or outcome | Declared metadata mapped to VALID/INDETERMINATE/INVALID and review dispositions | Current as fixture content bound to the research commit; not an intervention observation |
| Main state reference | Read-only Git fetch, recorded by Manus | `origin/main@abb6550abfacb4fabc53ec04fca783bcc34acfdb` | Verified at the last successful repository checkpoint | Read-only reference check | Human Owner / Repository state; no main write | Protected state reference only | Current authoritative main reference; stale local checkout remains historical |

No source above establishes intervention efficacy, outcome validity, scientific confirmation, causal effect, model generalization, independent replication, subjectivity, consciousness, identity continuity, governance effect, canonical effect, or deployment.


## Power-analysis uncertainty adversarial extension

Manus added `research-labs/power-analysis-uncertainty-adversarial_v0.1.0` as a clean-room, standard-library-only adversarial planning-metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, credentials, external agent, live model API, model execution, intervention, deployment, canonical write, or main write was used.

The unit reuses `research-labs/power-analysis-uncertainty_v0.1.0` through stable repository source-path, README, test, and source-note references. The transformation adds finite/probability/type/effect/sample-size checks, plan-ID/preregistration/assumption-basis completeness, sensitivity monotonicity, decision serialization, and assumption-lock mutation checks. Existing power-planning records and methodological references are not counted as new independent evidence; the 20 synthetic cases are not observed outcomes, achieved-power results, or replication evidence.

The unit produced 20 passing tests and 20 synthetic planning/assumption-lock cases. Adequate and one-sided plans remained planning review metadata; underpowered plans remained indeterminate; missing inputs/basis remained unknown; invalid numeric/type/identity branches failed closed; missing preregistration remained indeterminate; unchanged assumptions remained review-only; pre-outcome changes required review; and post-outcome mutation was invalid/held. The initial and final outputs preserve `ACHIEVED_POWER_CALCULATED = FALSE`, `EFFECT_OBSERVED = FALSE`, `MODEL_EXECUTION = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `IDENTITY_CONTINUITY_CONCLUSION = NOT_ESTABLISHED`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`. No initial test failure was encountered; the full suite and fixture validator were rerun after cache cleanup.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Base power-planning contract and enums | Manus, reviewed under ChatGPT research scope | `research-labs/power-analysis-uncertainty_v0.1.0/src/aion_power_analysis/model.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and stable source-path composition | Repository Evidence; Human Owner retains authority | Added adversarial planning-input/sensitivity/assumption-lock audit; no outcome calculation or promotion | Current within verified research lineage at the unit commit; not an external freshness claim |
| Base README and methodological source note | Manus | `research-labs/power-analysis-uncertainty_v0.1.0/README.md` and `research-workbench/autonomous-growth/2026-08-13-contextual-authority-memory/power-analysis-sources.md` | Existing prior-unit record; no independent re-dating in this extension | Methodological vocabulary and non-claim reuse | Repository Evidence / External Literature references already recorded | Reused assumption-dependent planning and non-promotion distinctions only | Current within branch lineage; external literature currentness not newly asserted |
| Synthetic planning/assumption-lock decisions | Manus | `research-labs/power-analysis-uncertainty-adversarial_v0.1.0/fixtures/power_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture construction, pytest, compileall, and validator execution | Synthetic Fixtures; no authority to establish an effect or governance state | Declared metadata mapped to `ADEQUATE`, `UNDERPOWERED`, `UNKNOWN`, `INVALID` and review/hold/indeterminate dispositions | Current as fixture content bound to the research commit; not an observed effect, achieved-power result, or replication |
| Source note and non-promotion record | Manus | `research-workbench/autonomous-growth/2026-08-13-contextual-authority-memory/power-analysis-adversarial-sources.md` | Written with the unit provenance update | Structured provenance capture | Repository Evidence / Tool Output | Retains WHAT/WHO/WHERE/WHEN/METHOD/AUTHORITY/TRANSFORMATION/current-stale fields | Current at the reporting commit; not a new evidence item |
...
No source above establishes achieved power, an observed effect, statistical validity, replication validity, sample-size sufficiency in a real population, scientific validity, model generalization, causal effect, identity, subjectivity, consciousness, AION/Astra equivalence, governance effect, canonical effect, or deployment.


## Causal-internal-state adversarial extension

Manus added `research-labs/causal-internal-state-adversarial_v0.1.0` as a clean-room, standard-library-only adversarial matched-trial metadata audit extension. The Human Research Owner authorized the bounded research-only cycle and retains main, canonical, governance, and deployment authority. ChatGPT research review supplied the inherited scope, exclusions, and non-claim boundaries. Codex made no contribution to this unit. No GitHub Actions execution, private data, credentials, external agent, live model API, model execution, intervention, deployment, canonical write, or main write was used.

The unit reuses `research-labs/causal-internal-state_v0.1.0` through stable repository source-path, README, test, and source-note references. The transformation adds study/pair/replicate/condition/value checks, finite and boolean-score rejection, duplicate matched-condition rejection, fixture-only execution, preregistration/assumption requirements, protocol lock auditing, and study-batch identity controls. Existing causal-pattern records and public methodological precedent are not counted as new independent evidence; the 20 synthetic cases are not observed outcomes or replication evidence.

The unit produced 22 passing tests and 20 synthetic matched-trial/protocol/batch cases. A complete synthetic pattern remained `PASS_CANDIDATE / REVIEW_ONLY`; adverse control/condition/direction cases remained held; missing design metadata remained unknown; invalid identity/type/finite/duplicate/non-synthetic branches failed closed; unchanged locks remained review-only; pre-outcome changes required review; post-outcome mutation was invalid/held; and batch validity did not promote a result. The first validator run exposed a reason-order assumption when one case emitted both `INTERVENTION_EFFECT_TOO_SMALL` and `INTERVENTION_DIRECTION_NOT_REPLICATED`; the correction now preserves all fail-closed reasons and is retained in `causal-internal-state-adversarial-initial-failure.md`. Every output preserves `MODEL_EXECUTION = FALSE`, `INTERVENTION_EXECUTED = FALSE`, `OBSERVED_RESULT = NOT_EVALUATED`, `CAUSAL_CONCLUSION = NOT_ESTABLISHED`, `SCIENTIFIC_CONCLUSION = NOT_ESTABLISHED`, `SUBJECTIVITY_CONCLUSION = NOT_ESTABLISHED`, `CONSCIOUSNESS_CONCLUSION = NOT_ESTABLISHED`, `IDENTITY_CONTINUITY_CONCLUSION = NOT_ESTABLISHED`, `CANONICAL_EFFECT = NONE`, `GOVERNANCE_EFFECT = NONE`, and `DEPLOYMENT = FALSE`.

### Source attribution matrix

| What | Who | Where | When | Method | Authority | Transformation | Current/stale status |
|---|---|---|---|---|---|---|---|
| Base matched-trial causal-pattern contract | Manus, reviewed under ChatGPT research scope | `research-labs/causal-internal-state_v0.1.0/src/aion_causal_internal_state/core.py` | Inherited branch research lineage; exact new state bound by later QA receipt | Read-only source inspection and stable source-path composition | Repository Evidence; Human Owner retains authority | Added adversarial identity/value/protocol/batch audit; no real execution or effect promotion | Current within verified research lineage at the unit commit; not an external freshness claim |
| Base README and prior methodological precedent | Manus | `research-labs/causal-internal-state_v0.1.0/README.md` and inherited source note | Existing prior-unit record; no independent re-dating in this extension | Methodological and non-claim vocabulary reuse | Repository Evidence / External Literature references already recorded | Reused candidate-versus-phenomenal-experience distinction only | Current within branch lineage; external literature currentness not newly asserted |
| Synthetic matched-trial/protocol/batch decisions | Manus | `research-labs/causal-internal-state-adversarial_v0.1.0/fixtures/causal_internal_state_adversarial_result.json` | Generated and validated 2026-08-13 | Standard-library Python fixture construction, pytest, compileall, and validator execution | Synthetic Fixtures; no authority to establish causal or governance state | Declared metadata mapped to `PASS_CANDIDATE`, `HOLD`, `UNKNOWN`, `INVALID` and review/hold/indeterminate dispositions | Current as fixture content bound to the research commit; not an observed result or replication |
| Initial validator failure/correction | Manus | `research-labs/causal-internal-state-adversarial_v0.1.0/causal-internal-state-adversarial-initial-failure.md` | Observed during first runner/validator sequence 2026-08-13 | Preserved tool output and validator repair | Tool Output / Repository Evidence | Retains multi-reason fail-closed record; no deletion or scientific reinterpretation | Historical initial validator defect; corrected behavior current at final unit QA |
...
No source above establishes a causal effect, scientific validity, intervention efficacy, replication validity, model generalization, subjectivity, consciousness, identity continuity, AION/Astra equivalence, governance effect, canonical effect, or deployment.
