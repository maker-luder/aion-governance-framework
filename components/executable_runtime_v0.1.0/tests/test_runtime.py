...
from aion_astra_runtime import AstraRuntime, RunStatus, TaskSpec
...
class RuntimeEndToEndTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.baseline.mkdir()
        self.sessions.mkdir()
        (self.baseline / "input.txt").write_text("Astra runtime test\nSecond line\n", encoding="utf-8")

    def tearDown(self) -> None:
        self.temp.cleanup()

    def test_complete_candidate_loop_preserves_baseline(self) -> None:
        before = (self.baseline / "input.txt").read_bytes()
        result = AstraRuntime().run(task(), baseline_root=self.baseline, sessions_root=self.sessions)
        ...
        self.assertFalse(record["deployment"])
        self.assertEqual(record["subjectivity_conclusion"], "NOT_ESTABLISHED")

    def test_kill_switch_holds_before_first_action(self) -> None:
        kill = Path(self.temp.name) / "KILL"
        kill.write_text("stop", encoding="utf-8")
        result = AstraRuntime().run(task("RUNTIME-KILL-001"), baseline_root=self.baseline, sessions_root=self.sessions, kill_switch=kill)
    ...
    def test_step_budget_exhaustion_holds(self) -> None:
        ...
        result = AstraRuntime(RepeatingPlanner()).run(task("RUNTIME-BUDGET-001"), baseline_root=self.baseline, sessions_root=self.sessions)
        self.assertEqual(result.status, RunStatus.HOLD)
