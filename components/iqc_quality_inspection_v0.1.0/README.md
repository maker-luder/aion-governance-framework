...
The research branch uses the same command with `--expected-targets 46`. The target count is branch-native and must come from the current component runner rather than from a historical snapshot.
