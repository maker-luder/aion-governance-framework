...
QA={'qa/CURRENT_TEST_RESULTS.json','qa/CURRENT_RELEASE_STATUS_LOCK.json','qa/TEST_RESULTS.md','qa/CURRENT_QA_RECONCILIATION.json','qa/CURRENT_COVERAGE_RESULTS.json','qa/CURRENT_COVERAGE_EVIDENCE.json','qa/COVERAGE_REPORT.md','qa/CURRENT_EVIDENCE_TRACEABILITY.json','qa/IQC_REPORT.json'}
def J(p):
...
def coverage(root,head):
    ...
    _,_,_,ts=summary(t); cs={str(i.get('target',i.get('relative_target',''))) for i in c if isinstance(i,dict)}-{''}
...
def source(root,head):
    ...
    q=sorted(x for x in dirty if x not in QA)
