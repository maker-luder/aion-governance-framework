...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Governed AION individual runtime candidate")
    result.add_argument("--memory-db", type=Path, default=Path("runtime_sessions/aion_memory.sqlite3"))
    result.add_argument("--runtime-instance-id", required=True)
    result.add_argument("--memory-stream-id", required=True)
    result.add_argument("--event-lineage-id", required=True)
    result.add_argument("--canonical-state-reference", required=True)
    result.add_argument("--genesis-root-id", required=True)
    ...
    serve_cmd.add_argument("--allow-non-loopback", action="store_true")
...
def _context(args: argparse.Namespace) -> IndividualRuntimeContext:
    return IndividualRuntimeContext(
        agent_id="AION",
        runtime_instance_id=args.runtime_instance_id,
        memory_stream_id=args.memory_stream_id,
        event_lineage_id=args.event_lineage_id,
        canonical_state_reference=args.canonical_state_reference,
        genesis_root_id=args.genesis_root_id,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    recalled = runtime.recall(
        user_id=args.user_id,
        requester_scopes=args.scope,
        entity_cues=args.entity,
        topic_cues=args.topic,
        limit=args.limit,
    )
    print(
        json.dumps(
