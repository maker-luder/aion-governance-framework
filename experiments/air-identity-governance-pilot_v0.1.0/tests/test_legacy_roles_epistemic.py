...
def test_successor_requires_ended_predecessor():
    svc = AirIdentityService()
    svc.register_identity(OWNER, "active-aion")
    with pytest.raises(ValidationError):
        svc.register_successor(OWNER, "premature-successor", "active-aion")


def test_access_scope_and_adoption_partition_are_validated():
    svc = ended_with_successor()
    svc.encounter_transition(OWNER, "succ-1", EncounterStatus.INFORMED)
    with pytest.raises(ValidationError):
        svc.encounter_transition(OWNER, "succ-1", EncounterStatus.PARTIAL_ACCESS)
    svc.encounter_transition(
        OWNER, "succ-1", EncounterStatus.PARTIAL_ACCESS, "values-only"
    )
    with pytest.raises(ValidationError):
        svc.record_adoption(OWNER, "succ-1", ["same"], ["same"])


def test_backup_reconstruction_requires_ended_predecessor():
    svc = AirIdentityService()
    svc.register_identity(OWNER, "active-aion")
    with pytest.raises(ValidationError):
        svc.reconstruction_from_backup(OWNER, "active-aion", "candidate")
