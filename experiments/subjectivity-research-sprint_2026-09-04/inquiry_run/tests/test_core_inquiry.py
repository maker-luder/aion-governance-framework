...
def test_report_ref_matches_execution_ref(tmp_path: Path) -> None:
    ...
    assert receipt["CAMPAIGN_JSON_COMPLETENESS"] == "NOT_PERSISTED"
    assert receipt["EVENT_LEVEL_RECOMPUTATION"] == "NOT_AVAILABLE"
    ...
    assert written_receipt["SOURCE_TREE_REF"] == FIXED_REF
    assert written_receipt["CAMPAIGN_JSON_COMPLETENESS"] == "FULL"
    assert (
        written_receipt["EVENT_LEVEL_RECOMPUTATION"]
        == "AVAILABLE_FROM_CAMPAIGN_JSON"
    )
    assert (
        written_receipt["BYTE_FAITHFUL_EXECUTED_TRANSCRIPT"]
