...
def execution_receipt(
    campaign: InquiryCampaignReport,
    *,
    source_tree_ref: str,
    persisted_full_artifact: bool = False,
) -> dict[str, str]:
    ...
    return {
        ...
        "EVENT_LEVEL_RECOMPUTATION": (
            "AVAILABLE_FROM_CAMPAIGN_JSON"
            if persisted_full_artifact
            else "NOT_AVAILABLE"
        ),
        "BYTE_FAITHFUL_EXECUTED_TRANSCRIPT": (
            "PRESERVED_IN_CAMPAIGN_JSON"
            if persisted_full_artifact
            else "NOT_PERSISTED"
        ),
        "CAMPAIGN_JSON_COMPLETENESS": (
            "FULL" if persisted_full_artifact else "NOT_PERSISTED"
        ),
...
def write_derived_artifacts(campaign: InquiryCampaignReport, destination: Path) -> tuple[Path, Path, Path]:
    ...
    receipt = execution_receipt(
        campaign,
        source_tree_ref=campaign.repository_ref,
        persisted_full_artifact=True,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_sandbox_derived:
        _, _, receipt_path = write_derived_artifacts(campaign, HERE)
