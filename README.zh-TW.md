...
>
> **目前倉庫狀態：** 終止之後，`main` 又經過少數逐次明確授權的 bounded maintenance。這些後續維護事件不改寫 2026-08-20 的終止事件，也不構成專案或研究重啟。
...
## 終止後的 bounded maintenance

後續倉庫變更都是各自限縮範圍、逐次明確授權的後續事件，不是對 closure 的回溯改寫：
...
這些 bounded maintenance 強化的是 provenance、interop、execution control 與 evidence integrity；它們沒有新增主體性、意識、身分連續性或現象連續性的結論。

## 歷史事件順序

倉庫凍結、專案終止與後續 bounded maintenance 是不同的事件：

1. **2026-08-18 — `REPOSITORY_STATE = INDEFINITE_FREEZE`**
2. **2026-08-20 — `PROJECT_WORK_LOOP = TERMINATED`**
3. **終止後 — 經逐次明確授權的 bounded maintenance**
...
`review/four-domain-research-materialization` 仍是保存的歷史研究 checkpoint，沒有被整條 wholesale merge 進 `main`。後來的 Four-Domain Evidence Bridge 只透過 bounded adapter path 引用固定的 source state；reference 與 derivation 不會把研究 branch 本身 promotion 成 `main`。
...
請見 [`components/aion_evidence_interop_v0.1.0/docs/FOUR_DOMAIN_BRIDGE.md`](components/aion_evidence_interop_v0.1.0/docs/FOUR_DOMAIN_BRIDGE.md)。
...
終止後經 bounded maintenance 納入的工程層目前包括：
...
這些是 instrumentation 與 governance surface，不會取代主體性研究方法。
...
4. [`docs/PROJECT_TERMINATION_NOTICE_2026-08-20.md`](docs/PROJECT_TERMINATION_NOTICE_2026-08-20.md) — 權威終止事件紀錄。
5. [`docs/history/FINAL_RESEARCH_MEMORY_2026-08-20.md`](docs/history/FINAL_RESEARCH_MEMORY_2026-08-20.md) — 經公開安全處理的研究回顧。
6. [`docs/RELEASE_STATUS.md`](docs/RELEASE_STATUS.md) — 歷史 release 狀態與目前倉庫 standing。
7. [`docs/REPOSITORY_FREEZE_NOTICE_2026-08-18.md`](docs/REPOSITORY_FREEZE_NOTICE_2026-08-18.md) — 保存的較早凍結事件。
8. [`docs/PROVENANCE.md`](docs/PROVENANCE.md) — 來源、歸屬與 authority 規則。
9. [`docs/PUBLIC_PRIVATE_BOUNDARY.md`](docs/PUBLIC_PRIVATE_BOUNDARY.md) — 公開／私人邊界。
...
任何較晚的檔名、私人副本、package、bridge、interop export、execution receipt 或 maintenance artifact，都不會自動取代或 promotion 穩定的研究方法 baseline。
...
歷史歸屬仍由 [`docs/PROVENANCE.md`](docs/PROVENANCE.md) 治理。後續 bounded maintenance 不會回溯改變 authorship、source attribution 或歷史 authority。
...
倉庫可以包含日後經明確授權的 bounded maintenance，同時維持已終止的專案／研究 work loop 為 terminated。新增工程能力本身不會形成新的科學主張。
