...
**目前工程狀態：** `main` 已包含具精確量測語意綁定的 bounded TEVV、Full-QMS、AI 對抗性安全設定檔與內容定址安全收據，以及用於來源拆分、區辨與 synthetic D2 × D4 測試的有限範圍 CCAP harness。這些是工程控制與測試夾具範圍的研究工具；**不代表**已建立安全有效性、D2／D4 支持、獨立科學驗證、主體性、意識或現象經驗。
